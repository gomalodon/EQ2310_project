...
BER = zeros(iter,length(EbN0_db));
tsamp_list = zeros(1, length(EbN0_db));
phihat_list = zeros(1, length(EbN0_db));

for bits = 1:iter
    nr_training_bits = (bits-1);
    fprintf("current iteration has %d training bits\n", nr_training_bits)
...
%         end
      ...
      for blk = 1:nr_blocks
        ...
        tsamp_list(bits, snr_point) = t_samp;
        ...
        phihat_list(bits, snr_point) = phihat;
...
%         end
    ...
    % Compute the BER. 
    BER(bits,:) = nr_errors / nr_data_bits / nr_blocks;
end
...
ylabel('Pe');
if save_plots
    saveas(gcf, "./images/BER_training_length.png");
end

for i=1:length(phihat_list(:,1))
    phihat_mean(i) = mean(phihat_list(i,:));
end
figure
plot(phihat_mean);
xlabel('nr\_training\_bits');
ylabel('phihat');
if save_plots
    saveas(gcf, "./images/phase_training_length.png");
end

for i=1:length(tsamp_list(:,1))
    tsamp_mean(i) = mean(tsamp_list(i,:));
end
figure
plot(tsamp_mean);
xlabel('nr\_training\_bits');
ylabel('t\_samp');
if save_plots
    saveas(gcf, "./images/tsamp_training_length.png");
end
